...
BOX_TOOL = {
    ...
    "options": [
        ...
        {"flag": "--no-git", "desc": "跳过 git add/commit/push"},
    ...
    "examples": [
        {"cmd": "pub_publish --msg fix null error", "desc": "拉代码→升级版本→更新 changelog→pub get→提交→发布"},
        {"cmd": "pub_publish --msg release notes --no-publish", "desc": "只提交不发布"},
        {"cmd": "pub_publish --msg try --dry-run", "desc": "预演一次，不做任何修改"},
    ],
    "docs": "src/box_tools/flutter/pub_publish.md",
}
...
def update_version(current_version: str, branch_version: str | None) -> str:
    ...
    if branch_version:
        # branch_version 只看 base，不带 build
        cmp = compare_versions(format_semver(major, minor, patch, None), branch_version)
        if cmp < 0:
...
def update_pubspec_preserve_format(pubspec_path: Path, *, dry_run: bool = False) -> tuple[str, str] | tuple[None, None]:
    if not pubspec_path.exists():
        print(f"❌ 找不到文件: {pubspec_path}")
        return None, None

    content = pubspec_path.read_text(encoding="utf-8")

    # 版本号支持 x.y.z 或 x.y.z+build；引号可选；保持原有引号
    pattern = (
