...
BOX_TOOL = {
    ...
    "examples": [
        ...
        {"cmd": "box_pub_upgrade --yes --no-commit", "desc": "直接升级（不提交）"},
        {"cmd": "box_pub_upgrade --private-host my.private.repo",
         "desc": "仅升级 url 含关键词的 hosted 私有依赖"},
    ],
    "docs": "README.md",
}
...
def _apply_version_in_block(block: list[str], new_version: str) -> tuple[
...
def apply_upgrades_to_pubspec(pubspec_file: str, upgrades: list[UpgradeItem]) -> tuple[
    bool, list[str]]:
...
def flutter_pub_get():
    ...
    proc = subprocess.run(["flutter", "pub", "get"], stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                          text=True)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("commit_message", nargs="?", default="up deps",
                   help="Git 提交信息（默认 up deps）")
