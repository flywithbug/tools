...
BOX_TOOL = {
    ...
    "examples": [
        {"cmd": "riverpod_gen", "desc": "交互输入类名与输出目录"},
        {"cmd": "riverpod_gen Product", "desc": "在当前目录生成 product_notifier.dart 与 product_state.c.dart"},
        {"cmd": "riverpod_gen product_item --out lib/features/product", "desc": "在指定目录生成 product_item_* 文件"},
        {"cmd": "riverpod_gen Product --force", "desc": "覆盖已存在文件"},
    ],
    "docs": "src/box_tools/flutter/riverpod_gen.md",
}
...
def normalize_to_pascal(name: str) -> str:
    ...
    # If already snake_case
    if "_" in name:
        base = re.sub(r"_+", "_", name.lower()).strip("_")
        pascal = snake_to_pascal(base)
    else:
        # Could be camel/pascal or single word.
        # If all lower, treat as single token -> Pascal
        if name.islower():
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="riverpod_gen",
        description="生成 Riverpod StateNotifier + State 模板（notifier/state）",
    )
    p.add_argument(
        "name",
        nargs="?",
...
def main(argv: list[str] | None = None) -> int:
    ...
    # default legacy unless explicitly modern
    use_legacy = True
    if args.modern:
        use_legacy = False
    elif args.legacy:
    ...
    # tiny hint for build_runner if copywith enabled
    if with_copywith:
