...
`pub_version` 用于升级 Flutter 项目 `pubspec.yaml` 中的 `version:` 字段，并可选自动执行 `git add / commit / push`。
...
`pub_version` 随工具集一起安装（仓库：`flywithbug/tools`）。
...
默认情况下，`pub_version` 会读取**当前目录**下的 `pubspec.yaml`：
...
默认情况下，`pub_version` 会执行以下 Git 操作：
...
```sh
pub_version minor --file /absolute/or/relative/path/to/pubspec.yaml
```
